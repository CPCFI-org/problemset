...
#define debug(x...)
#endif
void printOrder(int n, vector<int> prereqMap[100001], unordered_map<int,int> &inDegree){
    queue<int> q;   
    vector<int> order;
...
void solve(){
    ...
    for (size_t i = 0; i < numPrereqs; i++)
    {
        int a, b;
        cin>>a>>b;
        prereqMap[a].push_back(b);
        inDegree[b]++;
    }

    printOrder(numCourses, prereqMap, inDegree);
